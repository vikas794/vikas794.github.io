...
export const caseStudies: CaseStudy[] = [
  {
    ...
    decisions: [
      { decision: "Fan-out", optionA: "Per-user upstream socket", optionB: "Single socket + in-memory broadcast", chosen: "Single socket + in-memory pub/sub fan-out (STOMP simple broker)" },
    ...
    codeRef: "Tick-publish excerpt, annotated",
    code: {
      path: "BrokerWebSocketService.java",
      honesty: "Simplified illustration of the production pattern — structure faithful, details trimmed — not a verbatim paste.",
      lines: [
        { code: "// One socket in; the STOMP broker fans each tick out to every subscribed session." },
        { code: "void onTick(Tick tick) {" },
        { code: "  messagingTemplate.convertAndSend(", note: "Spring's in-memory simple broker owns the fan-out — no custom session loop." },
        { code: "    \"/topic/ticks/\" + tick.token(), tick);" },
        { code: "  listeners.forEach(l -> l.onTick(tick));", note: "In-process consumers (P&L, research) — not client sessions." },
        { code: "}" },
      ],
    },
    diagram: "fanout",
